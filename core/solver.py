				...
				if alternate and e % 2 == 1:
					...
					for i in range(n_iters_per_epoch):
						...
						feed_dict_g_loss = {
							...
							self.model.captions_two: generated_captions,
						...
						if (i+1) % self.print_every == 0:
							...
							for j, gc in enumerate(decoded):
								print "Generated caption %d: %s" %(j+1, gc)
							saver.save(sess, os.path.join(self.model_path, 'model-gen-mix'), global_step=500 + model_num)
							print "model-gen-mix-%s saved." %(model_num + 500)
							model_num += 1
